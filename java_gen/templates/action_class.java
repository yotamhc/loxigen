...
import org.openflow.protocol.OFWriter;
...
class ${impl_class} implements ${msg.interface_name} {
    ...
    // wire type: ${msg.wire_type(version)}
    private final static byte WIRE_TYPE = ${msg.wire_type(version)};
//:: if msg.is_fixed_length(version):
    private final static int LENGTH = ${msg.min_length(version) };
//:: else:
    private final static int MINIMUM_LENGTH = ${msg.min_length(version) };
//:: #endif
...
//:: #endfor
    ...
    public OFActionType getType() {
        return OFActionType.${msg.constant_name.replace("ACTION_", "")};
    }
...
//:: #endfor
    ...
    static class Reader implements OFMessageReader<${msg.interface_name}> {
        ...
        public ${msg.interface_name} readFrom(ChannelBuffer bb) throws OFParseError {
            ...
            if(type != WIRE_TYPE)
                throw new OFParseError("Wrong message type: Expected=${msg.constant_name}(" + WIRE_TYPE + "), got="+type);
...
//:: #endfor
                return new ${impl_class}(
                    ${",\n                      ".join(
                         prop.name for prop in msg.properties_for_version(version))}
                    );
        }
    }

    final static Writer WRITER = new Writer();
    static class Writer implements OFWriter<${impl_class}> {
        @Override
        public void write(ChannelBuffer bb, ${impl_class} message) {
//:: if not msg.is_fixed_length(version):
            int startIndex = bb.readerIndex();
//:: #end

            // wire type: ${msg.wire_type(version)}
            bb.writeByte(WIRE_TYPE);
            //:: if msg.is_fixed_length(version):
            // length: ${msg.min_length(version) } bytes (fixed)
            bb.writeShort(LENGTH);
//:: else:
            // placeholder for length to be inserted later
            bb.writeShort(0);
//:: #end
...
//:: if not msg.is_fixed_length(version):
            // update length field
            int length = bb.readerIndex() - startIndex;
