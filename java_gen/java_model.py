...
class JavaEnum(object):
    def __init__(self, c_name, entry_version_value_map):
        self.c_name = c_name
        self.name   = "OF" + java_utils.name_c_to_caps_camel("_".join(c_name.split("_")[1:]))
...
class JavaOFInterface(object):
    ...
    def __init__(self, c_name, version_map):
        ...
        self.name = java_utils.name_c_to_caps_camel(c_name)
    ...
    def class_info(self):
        if utils.class_is_message(self.c_name):
            return ("", "OFMessage")
...
class JavaMember(object):
    ...
    def enum_value(self):
        ...
        java_type = self.java_type.public_type;
        try:
            enum = build_java_model().enum_by_name(java_type)
    ...
    def for_of_member(java_class, member):
        if isinstance(member, OFPadMember):
            ...
            if member.name == 'len':
                ...
                name = java_utils.name_c_to_camel(member.name)
            java_type = java_utils.convert_to_jtype(java_class.c_name, member.name, member.oftype)
            return JavaMember(java_class, name, java_type, member)
